...
import { getMembersTickets } from "features/tickets/ticketActions";

function TicketChats() {
  const isOpen = useSelector((state) => state.ticket.modalIsOpen);
  const { userInfo } = useSelector((state) => state.auth);
  const { membersTickets } = useSelector((state) => state.ticket);
  ...
  useEffect(() => {
    dispatch(getMembersTickets(userInfo._id));
